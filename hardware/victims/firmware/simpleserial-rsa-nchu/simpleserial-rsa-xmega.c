...
// const uint8_t encrypted_x[] PROGMEM = {0x50, 0xb4, 0xc1};


uint8_t load_pt(bigint_t Mypt);
uint8_t get_en(uint8_t * pt, uint8_t len);
// uint8_t load_pt_2(bigint_t Mypt, uint8_t inputpt);


const uint8_t encrypted_key_temp[] PROGMEM = {
    0x50, 0xb4, 0xc1, 0x41, 0x36, 0xbd, 0x19, 0x8c, 0x2f, 0x3c, 0x3e, 0xd2, 0x43, 0xfc, 0xe0, 0x36
...
#define Mykey encrypted_key_temp
...
uint8_t load_bigint_from_seri(bigint_t* a, uint8_t seri, uint16_t length_B){
	a->length_B = BIGINT_CEIL(length_B) / sizeof(bigint_word_t);
	a->wordv = malloc(BIGINT_CEIL(length_B));
	if(!a->wordv){
        putch('F');
//         while(1);
	...
	memset(a->wordv, 0, sizeof(bigint_word_t));
	memcpy_P((uint8_t*)a->wordv + BIGINT_OFF(length_B), &seri, length_B);
	a->info = 0;
	bigint_changeendianess(a);
	bigint_adjust(a);
	return 0;
...
uint8_t load_pt(bigint_t Mypt){
    ...
    load_bigint_from_os(&Mypt, ENCRYPTED, sizeof(ENCRYPTED));
    return 0;
}
...
uint8_t get_pt(uint8_t * pt, uint8_t len)
{
    ...
    //型別不同導致錯誤，可到bigint.h查看詳細內容
    // bigint_t temp;
    // load_bigint_from_os(&temp, ENCRYPTED, sizeof(ENCRYPTED));
    const bigint_t* a = &temp;

    // 不做intialize可以使用，不過結果無法確定。
    // const bigint_t* a;
...
//     load_bigint_from_os(&Mypt, inputpt, sizeof(inputpt));
//     return 0;
// }
...
uint8_t get_en(uint8_t * pt, uint8_t len)
{
    bigint_t Mypt;
    uint8_t Mytest = 12;
    // Mytest = load_bigint_from_os(&Mypt, ENCRYPTED, sizeof(ENCRYPTED));
    Mytest = load_bigint_from_seri(&Mypt, *pt, sizeof(*pt));
	uint8_t flag = 0;
    ...
    uint8_t encrypted_key_temp_2[] = {
        0xa5, 0xb4, 0xc1, 0x41, 0x36, 0xbd, 0x19, 0x8c, 0x2f, 0x3c, 0x3e, 0xd2, 0x43, 0xfc, 0xe0, 0x36
